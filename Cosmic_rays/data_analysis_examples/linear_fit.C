...
void fit(std::vector<double> centroids, std::vector<double> energies) {
    //Please add errors!!

	double m=1., q=0.; // default values for calibration (m=1, q=0 means "no calibration")

	// Define a TGraphErrors which contains the experimental points in x
	// and the theoretical points in y; I semplify by not taking into account errors.
	TGraphErrors *graphErr = new TGraphErrors(centroids.size(),&centroids[0],&energies[0]);

	// Define a function which fits the points
	TF1 *fitfun = new TF1("calfitfun1","pol1",centroids.front(),centroids.back());
